...
from slrs.utils.conversions import (
    ev_to_nanometre,
    nanometre_to_ev,
    nanometre_to_wavenumber,
...
class Grid:
    ...
    def from_file(cls, path_to_configuration_file: str) -> None:
        ...
        if (
            minimum_wavelength_nm := parsed_configuration.get("minimum_wavelength_nm")
        ) is not None and (
            maximum_wavelength_nm := parsed_configuration.get("maximum_wavelength_nm")
        ) is not None:
            return cls(
        ...
        if (
            minimum_wavenumber_icm := parsed_configuration.get("minimum_wavenumber_icm")
        ) is not None and (
            maximum_wavenumber_icm := parsed_configuration.get("maximum_wavenumber_icm")
        ) is not None:
            return cls(
                parsed_configuration["minimum_wavevector_im"],
                parsed_configuration["maximum_wavevector_im"],
        ...
        if (
            minimum_energy_ev := parsed_configuration.get("minimum_energy_ev")
        ) is not None and (
            maximum_energy_ev := parsed_configuration.get("maximum_energy_ev")
        ) is not None:
            return cls(
                parsed_configuration["minimum_wavevector_im"],
                parsed_configuration["maximum_wavevector_im"],
    ...
    def wavevectors_im(self) -> npt.NDArray[np.float64]:
        return np.linspace(
            self.minimum_wavevector_im,
            self.maximum_wavevector_im,
            self.number_of_wavevectors,
        )
    ...
    def wavelengths_nm(self) -> npt.NDArray[np.float64]:
        ...
        match self.preferred_unit:
            case SpectralUnit.Nanometres:
                return np.linspace(
                    self.minimum_wavelength_nm,
                    self.maximum_wavelength_nm,
    ...
    def plot_grid(self) -> Tuple[npt.NDArray[np.float64], npt.NDArray[np.float64]]:
        ...
        match self.preferred_unit:
            ...
            case SpectralUnit.Wavenumbers:
                return in_plane_wavevectors_im[..., 0], nanometre_to_wavenumber(
                    wavelengths_nm
                )
            case SpectralUnit.ElectronVolts:
...
class Calculation:
    ...
    def _build_finite(self, extent: List[int]):
        ...
        single_particle_inverse_polarisability_tensor = (
            self.particle.inverse_polarisability_tensor(
                wavelengths_nm, self.lattice.background_index
            )
        )
        ...
        self.source_vector = self.field.generate(
            self.lattice, in_plane_wavevectors_im, wavelengths_nm
        )
    ...
    def _build_infinite(self):
        ...
        start = dt.now()
        logger.info("constructing single particle polarisability tensor")
        single_particle_inverse_polarisability = (
            self.particle.inverse_polarisability_tensor(
                wavelengths_nm, self.lattice.background_index
            )[:, 0, ...]
        )
        ...
        if self.lattice.symmetry == Symmetry.Honeycomb:
            self.single_particle_inverse_polarisability = np.block(
                [
                    [
                        single_particle_inverse_polarisability,
                        np.zeros_like(single_particle_inverse_polarisability),
                    ],
                    [
                        np.zeros_like(single_particle_inverse_polarisability),
                        single_particle_inverse_polarisability,
            ...
            for kk in range(solution_vector.shape[0]):
                solution_vector[kk, ll] = np.linalg.solve(
                    (
                        ...
                        - matrix_a[kk]
                        * unique_wavevectors_im[kk] ** 2  # / coupled_mode_index[] ** 2
                    ),
                    source_vector[kk, ll, ...],
                )
    ...
    def _solve_finite(self, extent: List[int]):
        ...
        self.solution_vector = np.zeros(
            (*wavevectors_im.shape, 3 * num_sites), dtype=np.complex128
        )
        ...
        set_num_threads(8)
        with ProgressBar(
            total=wavevectors_im.shape[1], dynamic_ncols=True, leave=False
        ) as progress:
    ...
    def _solve_infinite(self):
        ...
        if (positions_in_cell_nm := self.lattice.positions_in_cell()) is None:
            self.solution_vector = np.zeros(
                (*wavevectors_im.shape, 6), dtype=np.complex128
            )
        else:
            self.solution_vector = np.zeros(
                (*wavevectors_im.shape, 6 * (positions_in_cell_nm.shape[0] + 1)),
                dtype=np.complex128,
            )
    ...
    def _build(self):
        ...
        if (extent := self.lattice.finite_extent) is not None:
            ...
            logger.info("building problem for an infinite lattice")
            self._build_infinite()
            logger.info("solving problem for an infinite lattice")
            self._solve_infinite()

    def _extinction_finite(self, extent: List[int]) -> npt.NDArray[np.complex128]:
        _, wavelengths_nm = self.grid.generate()
        wavevectors_im = (
            2.0 * np.pi * self.lattice.background_index / (1e-9 * wavelengths_nm)
        )
        ...
        extinction = (
            ...
            * wavevectors_im
            * np.imag(
                np.sum(np.conj(self.source_vector) * self.solution_vector, axis=-1)
            )
    ...
    def _matrix_m(
        ...
        theta = np.arctan2(
            ...
            np.where(
                np.imag(out_of_plane_wavevectors_inm) == 0.0,
                np.real(out_of_plane_wavevectors_inm),
                0,
            ),
        )
        if reflectance:
    ...
    def _extinction_infinite(
        self, reflectance: bool = True
    ) -> npt.NDArray[np.complex128]:
        ...
        wavevectors_inm = (
            2 * np.pi / wavelengths_nm * self.lattice.coupled_mode_index(wavelengths_nm)
        )
        ...
        for m, n in product(range(-cutoff, cutoff + 1), range(-cutoff, cutoff + 1)):
            ...
            diffracted_order_in_plane_wavevectors_inm = (
                in_plane_wavevectors_inm + reciprocal_wavevectors_inm
            )
            ...
            # impl Eq 35
            summed_polarisation = (
                self.solution_vector[..., 0:6]
                * np.exp(
                    1j
                    * np.dot(
                        diffracted_order_in_plane_wavevectors_inm, evaluation_point_nm
                    )
                )[..., np.newaxis]
            )
            if (positions_in_cell_nm := self.lattice.positions_in_cell()) is not None:
                for ii, positions_nm in enumerate(positions_in_cell_nm):
                    summed_polarisation += (
                        self.solution_vector[..., 6 * (ii + 1) : 6 * (ii + 2)]
                        * np.exp(
                            1j
                            * np.dot(
                                diffracted_order_in_plane_wavevectors_inm,
                                evaluation_point_nm - positions_nm,
                            )
                        )[..., np.newaxis]
                    )
            result = (
            ...
            result = np.einsum("...ij,...j", m, result)

            scattered_far_field += np.where(
                np.imag(out_of_plane_wavevectors_inm)[..., np.newaxis] == 0.0,
                result,
                0.0,
            )

        # Add zero-order input field contribution
        self.scattered_far_field = (
            scattered_far_field * 0.0 + self.source_vector[..., :6]
        )
        self.total_source = self.source_vector[..., :6]

        if (positions_in_cell_nm := self.lattice.positions_in_cell()) is not None:
            for ii in range(positions_in_cell_nm.shape[0]):
                self.scattered_far_field += self.source_vector[
                    ..., 6 * (ii + 1) : 6 * (ii + 2)
                ]
                self.total_source += self.source_vector[
                    ..., 6 * (ii + 1) : 6 * (ii + 2)
                ]
        ...
        renormalised_denominator = np.where(denominator != 0.0, denominator, 1e10)
    ...
    def _extinction(self) -> npt.NDArray[np.complex128]:
        ...
        """
        if (extent := self.lattice.finite_extent) is not None:
            extinction = self._extinction_finite(extent) / (
                np.pi * (1e-9 * self.particle.radius_nm) ** 2
            )
        else:
    ...
    def effective_polarisabilities(self) -> npt.NDArray[np.complex128]:
        ...
        for ll in range(self.matrix_a.shape[1]):
            effective_polarisabilities[:, ll, ...] = np.linalg.inv(
                self.single_particle_inverse_polarisability - self.matrix_a[:, ll, ...]
            )
