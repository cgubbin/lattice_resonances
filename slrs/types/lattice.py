...
class Lattice:
    ...
    def __post_init__(self):
        ...
        match self.symmetry:
            case Symmetry.Rectangular:
                assert len(self.lengths_nm) == 2, "a rectangular lattice is characterised by two lengthr"
            ...
            case Symmetry.Hexagonal:
                assert len(self.lengths_nm) == 1, "a hexagonal lattice is characterised by a single length"
        ...
        # reduced_reciprocal_vectors = [
        #     2 * np.pi * np.matmul(r_mat, self.basis_vectors[1][:2]) / np.dot(
        #         self.basis_vectors[0][:2], np.matmul(r_mat, self.basis_vectors[1][:2])
        #     ),
        #     2 * np.pi * np.matmul(r_mat, self.basis_vectors[0][:2]) / np.dot(
        #         self.basis_vectors[1][:2], np.matmul(r_mat, self.basis_vectors[0][:2])
        #     ),
        # ]

        # self.reciprocal_vectors = [
        #     np.array([reduced_reciprocal_vectors[0][0], 0.0 * reduced_reciprocal_vectors[0][1], 0.0]),
        #     np.array([0.0 * reduced_reciprocal_vectors[1][0], reduced_reciprocal_vectors[1][1], 0.0]),
        # ]
    ...
    def coupled_mode_index(
        ...
        if self.spp_coupling:
            ...
            return np.ones_like(wavelengths_nm, dtype=np.complex128) * self.background_index
