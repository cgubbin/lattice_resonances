...
# Where is illuminated
#
# NOTE: At the moment only `EnsembleAverage` gives useful data. This assumes all cells
# in the system are illuminated.
# Other variants look at one pillar in the requested area which works poorly. I think
# these should sum over a small number of pillars instead, but this is not implemented atm.
class Probe(Enum):
    Corner = 0
    Edge = 1
    Centre = 2
...
class Field:
    ...
    def _te_field(
        ...
        assert (
            slice[..., 0].shape == exponent.shape
        ), "slice and exponent size must match"
    ...
    def _right_circular_field(
        ...
        slice[..., 0] = (
            1
            / np.sqrt(2)
            * (np.cos(phi) * np.cos(theta) + 1j * np.sin(phi))
            * np.exp(exponent)
        )
        slice[..., 1] = (
            1
            / np.sqrt(2)
            * (np.sin(phi) * np.cos(theta) - 1j * np.cos(phi))
            * np.exp(exponent)
        )
    ...
    def _fill_field(
        ...
        match polarisation:
            case Polarisation.TE:
                Field._te_field(slice, phi, exponent)
            case Polarisation.TM:
                Field._tm_field(slice, theta, phi, exponent)
            case Polarisation.RightCircular:
                Field._right_circular_field(slice, theta, phi, exponent)
            case Polarisation.LeftCircular:
                Field._left_circular_field(slice, theta, phi, exponent)
    ...
    def _finite(
        ...
        wavevectors_im = (
            2.0 * np.pi / (wavelengths_nm * 1e-9) * lattice.background_index
        )
        out_of_plane_wavevectors_im = np.emath.sqrt(
            wavevectors_im**2 - np.linalg.norm(in_plane_wavevectors_im, axis=-1) ** 2
        )
        # angle of in-plane propagation in spherical coordinates
        phi = np.arctan2(
            in_plane_wavevectors_im[..., 1], in_plane_wavevectors_im[..., 0]
        )
        # azimuthal angle from surface normal
        theta = np.arctan2(
            np.sqrt(
                in_plane_wavevectors_im[..., 0] ** 2
                + in_plane_wavevectors_im[..., 1] ** 2
            ),
            np.where(
                np.imag(out_of_plane_wavevectors_im) == 0.0,
                np.real(out_of_plane_wavevectors_im),
                0.0,
            ),
        )
        ...
        electric_field = np.zeros(
            (*wavelengths_nm.shape, particle_count * 3), dtype=np.complex128
        )
            ...
            case Probe.Centre:
                if lattice.finite_extent[0] % 2 == 1:
                    ...
                    if lattice.finite_extent[1] % 2 == 1:
                        iis = [
                            lattice.finite_extent[0] // 2
                            + lattice.finite_extent[1] // 2 * lattice.finite_extent[0]
                        ]
                    else:
                        iis = [
                            lattice.finite_extent[0] // 2
                            + (lattice.finite_extent[1] // 2 - 1)
                            * lattice.finite_extent[0],
                            lattice.finite_extent[0] // 2
                            + lattice.finite_extent[1] // 2 * lattice.finite_extent[0],
                        ]
                else:
        ...
        electric_field = np.where(
            theta[..., np.newaxis] != np.pi / 2, electric_field, 0.0
        )
    ...
    def _infinite(
        ...
        wavevectors_im = (
            ...
            * np.pi
            / (wavelengths_nm * 1e-9)
            * lattice.coupled_mode_index(wavelengths_nm)
        )
        out_of_plane_wavevectors_im = np.emath.sqrt(
            wavevectors_im**2 - np.linalg.norm(in_plane_wavevectors_im, axis=-1) ** 2
        )
        ...
        match self.polarisation:
            ...
            case Polarisation.RightCircular:
                stacked_field[..., 0] = (
                    1 / np.sqrt(2) * (np.cos(phi) * np.cos(theta) + 1j * np.sin(phi))
                )
                stacked_field[..., 1] = (
                    1 / np.sqrt(2) * (np.sin(phi) * np.cos(theta) - 1j * np.cos(phi))
                )
        ...
        stacked_field = np.where(
            theta[..., np.newaxis] != np.pi / 2, stacked_field, 0.0
        )
            ...
            for positions_nm in positions_in_cell_nm:
                stacked_field = np.concatenate(
                    [
                        ...
                        stacked_field
                        * np.exp(
                            1j * np.dot(in_plane_wavevectors_im, 1e-9 * positions_nm)
                        )[..., np.newaxis],
                    ],
                    axis=-1,
                )
        ...
        match self.probe:
            case Probe.EnsembleAverage:
                if (particle_count := lattice.particle_count()) is not None:
                    ...
                    raise ValueError(
                        "infinite particles are illuminated, we cannot count them"
                    )
            case Probe.Corner:
                return 1
            case Probe.Edge:
